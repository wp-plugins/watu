...
<script type="text/javascript">
	function addGrade(){
		var newGrade = "<p><img class='gradeclose' onclick='jQuery(this).parent().remove();' src='<?php echo $GLOBALS['wpframe_plugin_folder'] ?>/close.jpg' alt='close' /><label>Grade Title:</label><input type='text' name='gradetitle[]' id='title' value='' /><br /><label>Description:</label><br /><textarea name='grade_description[]' rows='5'></textarea><br /><label>From (points):<input type='text' class='numeric' name='grade_from[]' value='' /></label><label>To (points):<input type='text'  name='grade_to[]' class='numeric' value='' /></label></p>";
	...
	function validate() {
		...
		if(!ret) alert('Please provide numeric values for the Grade from/to.');
		return ret;
	}
</script>
<div class="wrap">
...
<h3 class="hndle"><span><?php e('Exam Name') ?></span></h3>
<div class="inside">
...
<h3 class="hndle"><span><?php e('Grading') ?></span></h3>
<div class="inside" id="gradecontent">
<?php 
	foreach($grades as $row ) {
?>
<p><img class="gradeclose" onclick="jQuery(this).parent().remove();" src="<?php echo $GLOBALS['wpframe_plugin_folder'] ?>/close.jpg" alt='close' /><label>Grade Title:</label><input type='text' name='gradetitle[]' id="title" value='<?php echo stripslashes($row->gtitle); ?>' /><br /><label>Description:</label><br /><textarea name='grade_description[]' rows='5'><?php echo stripslashes($row->gdescription); ?></textarea><br /><label>Grade from:<input type='text' class='numeric' name='grade_from[]' value='<?php echo stripslashes($row->gfrom); ?>' /></label><label>Grade to:<input type='text' class='numeric' name='grade_to[]' value='<?php echo stripslashes($row->gto); ?>' /></label></p>
<?php }
if( count($grades)==0 ){
 ?>
 <p><img class="gradeclose" onclick="jQuery(this).parent().remove();" src="<?php echo $GLOBALS['wpframe_plugin_folder'] ?>/close.jpg" alt='close' /><label>Grade Title:</label><input type='text' name='gradetitle[]' id="title" value='' /><br /><label>Description:</label><br /><textarea name='grade_description[]' rows='5'></textarea><br /><label>Grade from:<input type='text' class='numeric' name='grade_from[]' value='' /></label><label>Grade to:<input type='text' class='numeric' name='grade_to[]' value='' /></label></p>
 <?php } ?>
</div><p><a href="javascript:;" onclick="addGrade()" style="padding:4px; margin-left:10px;">Add another grade</a></p></div>
